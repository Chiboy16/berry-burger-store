    ...
    if (text === "💰 Top Up Wallet") {
        return bot.sendMessage(chatId, `Select amount 👇`, { reply_markup: { keyboard: [["500", "1000", "2500"], ["5000", "7500", "10000"], ["20000"], ["⬅️ Back"]], resize_keyboard: true } });
    }

    if (["500", "1000", "2500", "5000", "7500", "10000", "20000"].includes(text)) {
        const amt = Number(text); const ref = "BB_" + Date.now() + "_" + chatId;
        db.payments[ref] = { userId: chatId, amount: amt, status: "pending" }; saveDB(db);
        return bot.sendMessage(chatId, `💳 **INVOICE GENERATED**\n\nAmount: **₦${amt}**`, {
            reply_markup: { inline_keyboard: [[{ text: "Proceed Payment ✅", url: `${process.env.KORA_CHECKOUT}?reference=${ref}&amount=${amt}` }]] }
        });
    }
});
EOF
...
console.log("🍓 Berry Burger Store - Advanced Architecture Live");
